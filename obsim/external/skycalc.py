...
class SkyCalcInterface(object):
    property_list = {
        ...
        'pwv' : {
            ...
            'default' : 3.5 * u.mm,
            'allowed' : np.array([0.05, 0.01, 0.25, 0.5, 1.0, 1.5, 2.5, 3.5, 5.0, 7.5, 10.0, 20.0, 30.0]) * u.mm,
            'output_unit' : u.mm,
        },
        'msolflux' : {
        ...
        'ecl_lon' : { # TBD: can this be calculated from observing_time + location?
            'unit' : u.deg,
            'default' : 135 * u.deg,
            'limits' : np.array([-180,180]) * u.deg,
            'output_unit' : u.deg,
        },
        'ecl_lat' : { # TBD: can this be calculated from observing_time + location?
            'unit' : u.deg,
            'default' : 90 * u.deg,
    ...
    def sun_position(self):
        ...
        if self._sun_position is None:
            ...
            self._sun_position = geocentric_sun_pos.transform_to(c.AltAz(obstime = self.observation_time, location = self.location))
    ...
    def sun_position(self, val):
        if val is not None:
            raise ValueError("Sun position is determined automatically and can only be reset to None.")
    ...
    def moon_position(self):
        ...
        if self._moon_position is None:
            ...
            self._moon_position = geocentric_moon_pos.transform_to(c.AltAz(obstime = self.observation_time, location = self.location))
    ...
    def moon_position(self, val):
        if val is not None:
            raise ValueError("Moon position is determined automatically and can only be reset to None.")
    ...
    def _call_skycalc_interface(self, verbose=False):
        call_list = ['skycalc_cli', '-i', self.data_path + self.parameter_filename, '-o', self.data_path + self.output_filename]
        ...
        try:
            ...
            raise RuntimeError("SkyCalc executable not found, either skycalc_cli is not installed or it is not on path.")
    
    def _get_output(self):
        if not os.path.exists(self.data_path + self.output_filename):
            raise RuntimeError("Can't find the created SkyCalc output file, either the output path is wrong or SkyCalc did not produce an output file.")
        ...
        flux = hdu[1].data['FLUX'] * (output_unit/wl).to(default_units.flux_wavelength_density)
    ...
    def _get_property(self, param):
        ...
        if 'allowed' in param_dict.keys():
            if val not in param_dict['allowed']:
                raise ValueError(f"Parameter '{param}' is set to '{val}', but must be one of {param_dict['allowed']}.")
        elif 'limits' in param_dict.keys():
            if val < param_dict['limits'][0] or val > param_dict['limits'][1]:
                raise ValueError(f"Value for parameter '{param}' is '{val}', which is outside the allowed range of '{param_dict['limits']}'.")
    ...
    def _make_val_str(self, val, param_dict):
        ...
        if isinstance(val, str):
            ...
            raw_value = (val / param_dict['output_unit']).to(u.dimensionless_unscaled)
    ...
    def _check_separation_constraint(self):
        ...
        if np.abs(z-z_moon) > rho or np.abs(z+z_moon) < rho:
            raise RuntimeError(f"The physical constraint |z-z_moon| <= rho <= |z+z_moon| is not satisfied. Create a bug report/contact a developer if this error appears.")
        if airmass < 1.0 or airmass > 3.0:
            raise RuntimeError(f"SkyCalc requires an airmass in the range [1.0, 3.0], but current pointing has an airmass of {airmass:.3g}.")
    ...
    def _build_input_file(self):
        ...
        for param in self.property_list.keys():
            if param not in ['airmass', 'moon_sun_sep', 'moon_target_sep', 'moon_alt', 'target_alt']:
        ...
        with open(self.data_path + self.parameter_filename, "w") as parameter_file:
            for p in output_list:
                parameter_file.write(p + '\n')
    
    def set_wavelengths(self, wavelengths, mode=None):
        '''
        Set minimum/maximum wavelength and wavelength step based on supplied set of wavelengths.
        '''
        ...
        if self.wgrid_mode == 'fixed_wavelength_step':
            ...
            self.wres = (wavelengths[:-1]/dlam).to_value(u.dimensionless_unscaled).max()
