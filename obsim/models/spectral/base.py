...
class SpectralModelSum(ModelSum):
    def __add__(self, m):
        if not isinstance(m, SpectralModel) \
                and not isinstance(m, SpectralModelSum):
            raise ValueError(f"Addition not supported between a Model and \
{type(m)}.")
...
class SpectralModel(Model):
    ...
    def __call__(self, wavelengths):
        if (hasattr(self, 'source')
                and hasattr(self.source, 'radial_velocity')) \
                and self.source.radial_velocity is not None:
